    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-3">
      <div className="w-full max-w-md rounded-2xl border border-[var(--border)] bg-[var(--surface-solid)] shadow-2xl text-[var(--text)]">
        ...
        <div className="p-4 space-y-3">
          {/* Ghi chú đơn */}
          <div>
            <label className="mb-1 block text-[11px] font-semibold text-[var(--muted)]">
              Ghi chú đơn hàng
            </label>
            <textarea
              rows={2}
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {[50_000, 100_000, 200_000, 500_000].map((amt) => (
              <button
                ...
                type="button"
                onClick={() =>
                  setCustomerPaid((p) => Math.min(1_000_000_000, p + amt))
                }
          ...
          <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-3 space-y-1.5 text-xs">
            {[
              ...
              { label: "Phụ thu", val: `+${fmtVND(surcharge)}đ`, cls: "" },
              {
                ...
                val: `${fmtVND(total)}đ`,
                cls: "font-bold text-emerald-500 text-sm",
              },
              {
                label: "Tiền khách đưa",
                val: `${fmtVND(customerPaid)}đ`,
                cls: "",
              },
              {
                label: "Tiền thừa",
                val: `${fmtVND(change)}đ`,
                cls: "font-bold text-emerald-500",
              },
            ].map((r) => (
...
function buildSnapshot(items) {
  return Object.fromEntries(
    items.map((i) => [
      i.id,
      { orderItemId: i.orderItemId ?? null, qty: i.qty, note: i.note ?? "" },
    ]),
  );
}
...
function snapshotEqual(a, b) {
  ...
  return keysA.every(
    (k) => b[k] && b[k].qty === a[k].qty && b[k].note === a[k].note,
  );
}
...
export function PosOrderColumn() {
  ...
  useEffect(() => {
    ...
    setHasUnsaved(
      !snapshotEqual(buildSnapshot(orderItems), lastSavedSnapshot.current),
    );
  }, [orderItems]);
  ...
  const handleDraftOrder = async () => {
    ...
    try {
      ...
      const currentOrderItemIds = new Set(
        existingItems.map((i) => i.orderItemId),
      );
  ...
  return (
    <>
      <section
        className={`flex h-full w-[28%] min-w-[260px] max-w-[360px] flex-col rounded-xl border
        ${
          theme === "dark"
      ...
      >
        ...
        <div className="shrink-0">
          {customer ? (
            <div
              className="mx-3 mt-2 flex items-center justify-between rounded-lg border
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {item.toppings?.map((t) => (
                      <p
                        key={t.variantId}
                        className="ml-1 text-[10px] text-[var(--muted)]"
                      >
                        + {t.variantName}
                      </p>
                    ))}
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-1.5">
                    ...
                    >
                      ...
                      <span className="min-w-[14px] text-center font-semibold text-[var(--text)]">
                        {item.qty}
                      </span>
                      <button
                        type="button"
            ...
            {[
              {
                label: "Gộp bàn",
                lock: true,
                act: () => window.alert("Gộp bàn: sẽ xử lý sau."),
                hover: "hover:border-amber-500 hover:text-amber-500",
              },
              {
                label: "Chuyển bàn",
                lock: true,
                act: () => setOpenChangeTable(true),
                hover: "hover:border-amber-500 hover:text-amber-500",
              },
              {
                label: "Hủy đơn",
                lock: false,
                act: () => window.alert("Hủy đơn: sẽ xử lý sau."),
                hover: "hover:border-rose-500 hover:text-rose-500",
              },
