...
function PaymentModal({
  open, onClose, total, customerPaid, onChangeCustomerPaid,
  discount, onChangeDiscount, surcharge, onChangeSurcharge, change,
}) {
  if (!open) return null
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="w-full max-w-lg rounded-xl border border-[var(--border)] bg-[var(--surface-solid)] p-4 text-[var(--text)] shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-slate-50">Thanh toán hóa đơn</h2>
            <p className="text-xs text-slate-400">Tổng {total.toLocaleString('vi-VN')}đ</p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-[1.2fr,1fr]">
          <div className="space-y-3">
            <label className="block text-xs text-slate-300">
              Số tiền khách đưa
              <input autoFocus type="number" value={customerPaid}
            ...
            <div className="flex gap-2 text-xs">
              <label className="flex-1">
                <span className="mb-1 block text-[11px] text-slate-300">Giảm giá</span>
              ...
              <label className="flex-1">
                <span className="mb-1 block text-[11px] text-slate-300">Phụ thu</span>
          ...
          <div className="space-y-2 rounded-lg border border-[var(--border)] bg-[var(--surface-2)] p-3 text-xs text-[var(--muted)]">
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-emerald-500">{total.toLocaleString('vi-VN')}đ</span>
            </div>
            <div className="flex justify-between">
              <span>Tiền khách đưa</span><span>{customerPaid.toLocaleString('vi-VN')}đ</span>
            </div>
            <div className="flex justify-between">
              <span>Giảm giá</span><span>{discount.toLocaleString('vi-VN')}đ</span>
            </div>
            <div className="flex justify-between">
              <span>Phụ thu</span><span>{surcharge.toLocaleString('vi-VN')}đ</span>
            </div>
            <div className="flex justify-between text-emerald-400">
              <span>Tiền thừa</span>
              <span className="font-semibold">{change.toLocaleString('vi-VN')}đ</span>
            </div>
            <div className="mt-3">
              <span className="mb-1 block text-xs text-slate-400">Phương thức thanh toán</span>
              <div className="flex flex-wrap gap-2">
...
function ChangeTableModal({ open, onClose }) {
  ...
  if (!open) return null
  const handleChange = (id) => { selectTable(id); onClose() }
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60">
      <div className="w-full max-w-sm rounded-xl border border-slate-800 bg-slate-900 p-3 text-xs text-slate-100">
        ...
        <p className="mb-2 text-[11px] text-slate-400">Chọn bàn muốn chuyển hoá đơn hiện tại sang.</p>
        <div className="grid grid-cols-3 gap-2">
          {tables.map((t) => (
            <button key={t.id} type="button" onClick={() => handleChange(t.id)}
              ...
              <span className="block text-xs font-semibold text-slate-50">Bàn {t.name}</span>
              <span className="block text-[10px] text-slate-400">
                {t.status === 'using' ? `Đang dùng · ${t.guests ?? 0} khách`
                  : t.status === 'reserved' ? 'Đã đặt chỗ' : 'Trống'}
              </span>
            </button>
          ))}
        </div>
...
export function PosOrderColumn() {
  ...
  const { selectedTable, orderItems, changeItemQty, subtotal,
        currentOrderId, setCurrentOrderId, updateItemNote  } = usePos()
...
const handleDraftOrder = async () => {
  ...
  return (
    <>
      <section className={`flex w-[28%] min-w-[260px] max-w-[360px] flex-col rounded-xl border ${
        theme === 'dark'
        ...
        <div className="flex-1 divide-y divide-[var(--border)] overflow-auto">
          {orderItems.map((item) => (
            <div key={item.id} className="flex items-start justify-between px-3 py-2 text-xs">
              <div className="flex-1">
                <p className="text-sm font-medium">
                  {item.productName} - {item.base?.variantName}
                </p>
                {item.toppings?.map((t) => (
                  <p key={t.variantId} className="ml-2 text-[11px] text-[var(--muted)]">
                    + {t.variantName}
                  </p>
                ))}
                <input
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                <div className="inline-flex items-center gap-1 rounded-full border border-[var(--border)] bg-[var(--surface-2)] px-1.5 py-0.5 text-[10px] text-[var(--text)]">
                  <button type="button" onClick={() => changeItemQty(item.id, -1)}>-</button>
                  <span>{item.qty}</span>
                  <button type="button" onClick={() => changeItemQty(item.id, 1)}>+</button>
                </div>
              </div>
        ...
        <div className="space-y-2 border-t border-[var(--border)] p-3">
          ...
          <div className="mb-2 flex flex-wrap gap-1 text-[10px]">
            ...
            <button type="button"
              ...
              onClick={() => window.alert('Gộp bàn sẽ được xử lý sau khi nối API.')}
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <label className="text-[11px] text-[var(--muted)]">
              ...
              <input type="number"
                className="mt-1 w-full rounded-lg border border-[var(--border)] bg-[var(--input)] px-2 py-1.5 text-xs text-[var(--text)]"
                value={surcharge} onChange={(e) => setSurcharge(Number(e.target.value) || 0)} />
            </label>
          </div>
          ...
          <label className="block text-[11px] text-[var(--muted)]">
            ...
            <input type="number"
              className="mt-1 w-full rounded-lg border border-[var(--border)] bg-[var(--input)] px-2 py-1.5 text-xs text-[var(--text)]"
              value={customerPaid} onChange={(e) => setCustomerPaid(Number(e.target.value) || 0)} />
          </label>
          ...
          <div className="mt-1 flex justify-between text-sm font-semibold text-emerald-500">
            <span>Tổng thanh toán</span><span>{total.toLocaleString('vi-VN')}đ</span>
          </div>
          <div className="flex justify-between text-[11px] text-emerald-600">
            <span>Thối lại</span><span>{change.toLocaleString('vi-VN')}đ</span>
          </div>
      ...
      <PaymentModal
        open={openPayment} onClose={() => setOpenPayment(false)}
        total={total} customerPaid={customerPaid}
        onChangeCustomerPaid={setCustomerPaid}
        discount={discount} onChangeDiscount={setDiscount}
        surcharge={surcharge} onChangeSurcharge={setSurcharge}
        change={change}
      />
      <ChangeTableModal open={openChangeTable} onClose={() => setOpenChangeTable(false)} />
    </>
  )
}